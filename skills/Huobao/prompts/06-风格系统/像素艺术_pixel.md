...
> **函数名**: `GetStylePrompt("pixel")`
> **源码位置**: `application/services/prompt_i18n.go:715-721 (中文) / 788-794 (英文)`
> **风格标识**: `pixel`
...
|------|------|
| 质感 | 清晰方格像素、阶梯状线条 |
| 色彩 | 受限调色盘、抖动算法（Dithering） |
| 光影 | 色块式阴影（Flat Shading） |
| 氛围 | 清爽、明快、电子游戏感 |
| 灵感来源 | 星露谷物语、经典 RPG |
...
```
Pixel art style, 8-bit/16-bit aesthetic, visible pixel grid,
aliased lines, staircase edges, no gradients, no blur,
limited color palette, dithering patterns, color block overlay,
medium saturation, crisp bright tones,
flat shading, pixel block shadows, constant lighting,
retro video game feel, Stardew Valley inspired
```
